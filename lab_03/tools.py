...
def GetAlgorithmPoints(window, x1, y1, x2, y2, color):
    ...
    if window.DDAAlg.isChecked():
        return DDAAlgorithm(x1, y1, x2, y2, color)
    elif window.WUAlg.isChecked():
        return WuAlgorithm(x1, y1, x2, y2, color)
    elif window.BresIntAlg.isChecked():
        return BresenhamIntegerAlgorithm(x1, y1, x2, y2, color)
    elif window.BresFloatAlg.isChecked():
        return BresenhamFloatAlgorithm(x1, y1, x2, y2, color)
    elif window.BresAntiAlg.isChecked():
        return BresenhamEliminationOfAliasingAlgorithm(x1, y1, x2, y2, color)
    else:
        return []
